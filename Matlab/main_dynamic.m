...
% Flag to simulate karma social welfare policy %%ALWAYS FALSE%%
...
if control.karma_heuristic_policies || (control.karma_ne_policies || control.karma_sw_policy) && param.karma_initialization ~= 2
    if param.karma_initialization == 0
        ...
        [s_up_k_uniform, K_uniform] = func.get_s_up_k_uniform_dynamic(param.k_ave);
...
if control.karma_ne_policies
    ...
    k_ne = cell(param.num_alpha, 1);
    file_str = ['karma_nash_equilibrium/results/dynamic/k_ave_', num2str(param.k_ave, '%02d'), '_alpha_'];
    for i_alpha = 1 : param.num_alpha
        file = [file_str, num2str(param.alpha(i_alpha), '%.2f'), '.mat'];
        load(file, 'ne_K');
        K_ne{i_alpha} = ne_K;
        ...
        if param.karma_initialization == 2
            ...
            % algorithm
            load(file, 'ne_s_up_k');
            k_ne{i_alpha}(1,:) = func.get_init_k(ne_s_up_k, K_ne{i_alpha}, param);
        else
    ...
    for inter = 1 : param.num_inter_per_day
        ...
        if control.karma_heuristic_policies
            ...
            if t < param.tot_num_inter
                [k_win, k_lose] = func.get_karma_payments_dynamic(m_win, param);
        ...
        if control.karma_ne_policies
            for i_alpha = 1 : param.num_alpha
                ...
                for i_agent = 1 : param.I_size
                    ...
                    i_k = min([k(i_agent) + 1, length(K_ne{i_alpha})]);
