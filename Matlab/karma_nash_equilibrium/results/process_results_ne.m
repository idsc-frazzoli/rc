    ...
    for i_alpha_comp = 1 : max([length(alpha_comp_vec), 1])
        for i_z = 1 : max([length(z1_vec), 1])
            ...
            load(file, 'param', 'ne_param', 'ne_pi_down_mu_alpha_u_k_up_b',...
                ...
                'ne_sigma_down_mu_alpha_up_k', 'ne_J_down_mu_alpha_u_k',...
                'ne_F_down_mu_alpha_u_k_b', 'ne_T_down_mu_alpha_u_k_up_un_kn',...
                'ne_pi_error_hist');
            ...
            if save_plots
                ...
                if ~exist('karma_nash_equilibrium/results/ne_karma_distributions', 'dir')
                    mkdir('karma_nash_equilibrium/results/ne_karma_distributions');
                end
                ...
                ne_sigma_file = ['karma_nash_equilibrium/results/ne_karma_distributions/k_bar_', num2str(k_bar, '%02d')];
                ...
                if length(alpha_comp_vec) >= 1
                    ...
                    ne_sigma_file = [ne_sigma_file, '_alpha_', alpha_str, '.png'];
                    ...
                    ne_pi_file = [ne_pi_file, '_z_', num2str(z1_vec(i_z), '%.2f'), '_', num2str(1 - z1_vec(i_z), '%.2f'), '.png'];
                    ne_d_file = [ne_d_file, '_z_', num2str(z1_vec(i_z), '%.2f'), '_', num2str(1 - z1_vec(i_z), '%.2f'), '.png'];
                    ne_sigma_file = [ne_sigma_file, '_z_', num2str(z1_vec(i_z), '%.2f'), '_', num2str(1 - z1_vec(i_z), '%.2f'), '.png'];
                    ne_J_file = [ne_J_file, '_z_', num2str(z1_vec(i_z), '%.2f'), '_', num2str(1 - z1_vec(i_z), '%.2f'), '.png'];
                    ne_T_file = [ne_T_file, '_z_', num2str(z1_vec(i_z), '%.2f'), '_', num2str(1 - z1_vec(i_z), '%.2f'), '.png'];
                    ne_pi_error_file = [ne_pi_error_file, '_z_', num2str(z1_vec(i_z), '%.2f'), '_', num2str(1 - z1_vec(i_z), '%.2f'), '.png'];
                end
                saveas(ne_pi_plot_fg, ne_pi_file);
                saveas(ne_d_plot_fg, ne_d_file);
                saveas(ne_sigma_plot_fg, ne_sigma_file);
