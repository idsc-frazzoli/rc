...
ne_file_header = ['karma_nash_equilibrium/results/dynamic/k_ave_'];

%% Fairness arrays
% USE VARIANCE OF KARMA DISTRIBUTION AS SURROGATE FOR FAIRNESS
ne_W2 = zeros(num_k_ave, num_alpha);

for i_k_ave = 1 : num_k_ave
    k_ave = k_ave_vec(i_k_ave);
    % NE fairnesses
    ne_file_str = [ne_file_header, num2str(k_ave, '%02d')];
    for i_alpha = 1 : num_alpha
        alpha = alpha_vec(i_alpha);
        ne_file = [ne_file_str, '_alpha_', num2str(alpha, '%.2f'), '.mat'];
        load(ne_file, 'ne_s_up_k', 'ne_K');
        ne_W2(i_k_ave,i_alpha) = dot(ne_s_up_k, ne_K.^2) - (dot(ne_s_up_k, ne_K))^2;
    end
end
