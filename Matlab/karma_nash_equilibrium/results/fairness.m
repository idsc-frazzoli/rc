...
for i_k_ave = 1 : num_k_ave
    ...
    for i_alpha = 1 : num_alpha
        ...
        load(ne_file, 'ne_d_up_u_k', 'ne_param');
        ne_W2(i_k_ave,i_alpha) = dot(sum(ne_d_up_u_k), ne_param.K.^2) - (dot(sum(ne_d_up_u_k), ne_param.K))^2;
    end
    % SW efficiencies
    if sw_computed
        sw_file = [sw_dir, '/k_ave_', num2str(k_ave, '%02d'), '.mat'];
        load(sw_file, 'sw_d_up_u_k', 'ne_param');
        sw_W2(i_k_ave) = dot(sum(sw_d_up_u_k), ne_param.K.^2) - (dot(sum(sw_d_up_u_k), ne_param.K))^2;
    end
end
