...
while true
    ...
    results_func.plot_ne_pi(ne_pi_plot_fg, ne_pi_plot_pos, RedColormap, pi_down_u_k_up_m, ne_param.U, ne_param.K, alpha);

    % NE stationary distribution plot
    ne_D_plot_fg = 2;
    ne_D_plot_pos = [0, screenheight / 2, screenwidth, screenheight / 2];
    results_func.plot_ne_D(ne_D_plot_fg, ne_D_plot_pos, D_up_u_k, ne_param.U, ne_param.K, alpha);
    
    % NE expected utility plot
    ne_V_plot_fg = 3;
    ne_V_plot_pos = [0, 0, screenwidth, screenheight / 2];
    results_func.plot_ne_V(ne_V_plot_fg, ne_V_plot_pos, V_down_u_k, ne_param.U, ne_param.K, alpha);
    
    % NE expected utiliy per message plot
    ne_V_m_plot_fg = 4;
    ne_V_m_plot_pos = [0, screenheight / 2, screenwidth, screenheight / 2];
    results_func.plot_ne_V_m(ne_V_m_plot_fg, ne_V_m_plot_pos, parula, V_down_u_k_m, ne_param.U, ne_param.K, alpha);
    
    % NE state transitions plot
    ne_T_plot_fg = 5;
    ne_T_plot_pos = [0, 0, screenwidth, screenheight];
    results_func.plot_ne_T(ne_T_plot_fg, ne_T_plot_pos, RedColormap, T_down_u_k_up_un_kn, ne_param.U, ne_param.K, alpha);
    
    % NE policy error plot
    ne_pi_error_plot_fg = 6;
    ne_pi_error_plot_pos = [0, 0, screenwidth, screenheight / 2];
    results_func.plot_ne_pi_error(ne_pi_error_plot_fg, ne_pi_error_plot_pos, ne_policy_error_hist, alpha);
