...
policy_plot_fg = 1;
policy_plot_pos = [0, 0, default_width, screenheight];
policy_plot_title = 'Current NE Guess Policy';
ne_func.plot_policy(policy_plot_fg, policy_plot_pos, policy, const, policy_plot_title, RedColormap);
...
theta_i = (eye(const.num_k) - ne_param.alpha * T_i) \ c_i;
...
rho_i = ne_func.get_rho(policy, D, theta_i, const, param, ne_param);
...
policy_i_plot_fg = 2;
policy_i_plot_pos = [default_width, 0, default_width, screenheight];
policy_i_plot_title = 'Best Response Policy';
ne_func.plot_policy(policy_i_plot_fg, policy_i_plot_pos, policy_i, const, policy_i_plot_title, RedColormap);

% Set next NE guess to best response, using momentum
policy_next = cell(const.num_k, 1);
for i_k_i = 1 : const.num_k
...
while policy_error > ne_param.policy_tol && num_iter < ne_param.ne_policy_max_iter
    ...
    fprintf('Iteration %d policy: %d', num_iter, policy_hist_end(1));
    for i_k_i = 2 : const.num_k
        policy_hist_end(i_k_i) = const.k(policy_i{i_k_i}==1);
        fprintf("->%d", policy_hist_end(i_k_i));
    end
    ...
    fprintf('\n\n');
    if limit_cycle && size(policy_limit_cycle, 1) > 1
